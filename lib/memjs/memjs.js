...
Client.prototype.get = function(key, callback) {
  ...
  this.perform(serv, request, function(error, response) {
    if (error) {
...
// INCREMENT  
//
// Takes a key and increments his value to memcache and a callback. The success of the
// operation is signaled through the argument to the callback. The last argument is
// an optional expiration which overrides the default expiration
...
// DECREMENT
//
// Takes a key to decrement from memcache and a callback. An error is passed as
// the first argument to the callback and the result of the
// operation is signaled through the argument to the callback.
...
Client.prototype.perform = function(serv, request, callback, retries) {
  ...
  var errorHandler = function(error) {
    if (--retries > 0) {
      ...
      var errorMessage = "MemJS: Server <" + serv.host + ":" + serv.port +
